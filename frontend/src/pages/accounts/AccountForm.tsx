                  ...
                  <FormControl fullWidth error={!!errors.subProductId} disabled={isLoading}>
                    ...
                    >
                      {subProductsData?.content
                        .filter((sp: SubProductResponseDTO) => sp.status === SubProductStatus.ACTIVE && sp.verified)
                        .map((subproduct: SubProductResponseDTO) => (
                          <MenuItem key={subproduct.subProductId} value={subproduct.subProductId}>
                            {subproduct.subProductName} ({subproduct.subProductCode}) - Int. Rate: {subproduct.interestRate}%
                          </MenuItem>
                        ))}
                    </Select>
          ...
          <FormSection title="SubProduct Details">
            <Grid container spacing={3}>
              <Grid item xs={12} md={4}>
                <TextField
                  label="Interest Rate"
                  value={`${selectedSubProduct.interestRate}%`}
                  fullWidth
              ...
              <Grid item xs={12} md={4}>
                <TextField
                  label="Term"
                  value={selectedSubProduct.term ? `${selectedSubProduct.term} ${selectedSubProduct.termCode || ''}` : 'N/A'}
                  fullWidth
                  InputProps={{ readOnly: true }}
                />
              </Grid>
